...
public static class DemoSections
{
    ...
    public static void PrintLuhnSection()
    {
        ...
        ConsoleHelper.PrintInfo("  Luhn just determines the last digit, so search space stays 10^9 per BIN");
        ConsoleHelper.PrintInfo("");
    ...
    public static void PrintBinSection()
    {
        ...
        Console.ResetColor();
        Console.WriteLine();
        ConsoleHelper.PrintHighlight("16-digit BINs (Visa / Mastercard)", $"{total16} -> {total16:N0} x 10^9 = {(long)total16 * 1_000_000_000L:N0} candidates");
        if (total15 > 0)
            ConsoleHelper.PrintHighlight("15-digit BINs (Amex)", $"{total15} -> {total15:N0} x 10^9 = {(long)total15 * 1_000_000_000L:N0} candidates");
        if (total14 > 0)
            ConsoleHelper.PrintHighlight("14-digit BINs (Diners)", $"{total14} -> {total14:N0} x 10^8 = {(long)total14 * 100_000_000L:N0} candidates");
        ConsoleHelper.PrintHighlight("Total search space (all 16-digit)", $"~{(long)total16 * 1_000_000_000L:N0}");
    }

    /// <param name="attacks">Results from the PAN cracking runs. HashesPerSecond is stored in M/sec.</param>
    /// <param name="isGpu">True when speeds should be displayed as G/sec.</param>
    public static void PrintTrack2Section(IReadOnlyList<AttackResult> attacks, bool isGpu = false)
    {
        ...
        Console.WriteLine();
        ConsoleHelper.PrintInfo("Expanded search space per known BIN:");
        ConsoleHelper.PrintInfo($"  PAN combinations                   :   {ResultSummary.PanCombinations:N0}  (10^9)");
        ConsoleHelper.PrintInfo($"  Expiry dates (6 yrs x 12 months)   :   {ResultSummary.ExpiryDatesAll} values");
        ...
        Console.WriteLine();
        ConsoleHelper.PrintHighlight("Full Track2 space per known BIN", $"{ResultSummary.Track2Full:N0}  (~10^12)");
        ConsoleHelper.PrintHighlight("With known expiry year", $"{ResultSummary.Track2KnownYear:N0}  (~1.8 x 10^11)");
        Console.WriteLine();

        ConsoleHelper.PrintInfo("Estimated crack time with known BIN, based on YOUR measured speeds:");
        Console.WriteLine();
        ...
        foreach (var a in attacks)
        {
            ...
            string fullTrack2 = ResultSummary.FormatDuration(ResultSummary.Track2Full / hps);
            string knownYear = ResultSummary.FormatDuration(ResultSummary.Track2KnownYear / hps);
            string speedLabel = isGpu
                ? $"{a.HashesPerSecond / 1000.0:F3} G/sec"
                : $"{a.HashesPerSecond:F1} M/sec";

            Console.ForegroundColor = ConsoleColor.White;
            Console.WriteLine($"  {a.Algorithm,-38} | {speedLabel,12} | {fullTrack2,15} | {knownYear,13}");
        }
        Console.ResetColor();
        Console.WriteLine();

        ConsoleHelper.PrintInfo($"If the 6-digit BIN is not known, the search is about {ResultSummary.UnknownBinMultiplier:N0}x larger:");
        ConsoleHelper.PrintHighlight("PAN only, no BIN", $"{ResultSummary.PanUnknownBin:N0} candidates");
        ConsoleHelper.PrintHighlight("Full Track2, no BIN", $"{ResultSummary.Track2FullUnknownBin:N0} candidates");
        ConsoleHelper.PrintHighlight("Known expiry year, no BIN", $"{ResultSummary.Track2KnownYearUnknownBin:N0} candidates");
        Console.WriteLine();

        Console.ForegroundColor = ConsoleColor.Yellow;
        Console.WriteLine($"  {"Algorithm",-38} | {"PAN only",15} | {"Full Track2",15} | {"Known year",13}");
        ...
        {
            double hps = a.HashesPerSecond * 1_000_000.0;
            string panOnly = ResultSummary.FormatDuration(ResultSummary.PanUnknownBin / hps);
            string fullTrack2 = ResultSummary.FormatDuration(ResultSummary.Track2FullUnknownBin / hps);
            string knownYear = ResultSummary.FormatDuration(ResultSummary.Track2KnownYearUnknownBin / hps);

            Console.ForegroundColor = ConsoleColor.White;
            Console.WriteLine($"  {a.Algorithm,-38} | {panOnly,15} | {fullTrack2,15} | {knownYear,13}");
        }
        Console.ResetColor();
        Console.WriteLine();
